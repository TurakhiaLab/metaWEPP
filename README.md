...
```
snakemake --config KRAKEN_DB=viral_kraken_db DIR=simulated_metagenomic_sample MIN_PROP=0.05 PATHOGENS=default,rsv_a,sars_cov_2 CLADE_LIST=,nextstrain,nextstrain:pango CLADE_IDX=-1,0,1 --cores 32
```
...
RSV-A Abundances
Argentina/BA-HNRG-369/2017|ON237340.1|2017-06-26,,0.500000
USA/MA-Broad_MGB-13815/2022|OQ171906.1|2022-07-27,,0.500000
```
...
```
snakemake --config KRAKEN_DB=viral_kraken_db DIR=real_metagenomic_sample MIN_PROP=0.05 PATHOGENS=default,rsv_a CLADE_LIST=,nextstrain CLADE_IDX=-1,0 --cores 32
```
...
d) Press Enter when it asks for MAT to use viral_usher for building MAT.
...
h) Press Enter when it asks for more fasta files, and metadata files.
i) Give "./data/pathogens_for_wepp/rhinovirus_a/viral_usher_build" as the directory for building MAT and press Enter.
```

**Step 6:**  Analyze Results.

Taxonomic classification results can be viewed in `results/real_metagenomic_sample/classification_proportions.png`, while haplotype-level results generated by WEPP for RSV-A and Rhinovirus-A are available in `WEPP/results/respiratory_syncytial_virus_a_simulated_metagenomic_sample` and `WEPP/results/rhinovirus_a_simulated_metagenomic_sample`, respectively.
