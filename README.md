...
**Step 1:** Download the MAT file for SARS-CoV-2.
```
wget https://hgdownload.gi.ucsc.edu/goldenPath/wuhCor1/UShER_SARS-CoV-2/2021/12/05/public-2021-12-05.all.masked.pb.gz
```
...
We assume that all samples are stored in the data directory, each within its own subdirectory given by DIR argument (see [Run Command](#run)). For each pathogen to be analyzed for variants with WEPP, place its reference genome and MAT file in the folder: `data/pathogens_for_wepp/<pathogen_name>`. The genomes file and MAT file can either be generated by metaWEPP or user provided. Moreover, each created `DIR` inside `data` is expected to contain either of these files.
1. Sequencing Reads: Ending with `*_R{1/2}.fastq.gz` or `*_R{1/2}.fastq` for paired-ended reads and `*.fastq.gz` or `*.fastq` for single-ended. (User Provided)
2. Genomes: Single `fasta` file containing all the genomes to be simulated in the sample.  (metaWEPP Generated or User Provided)
...
📁 metaWEPP
└───📁config
     ├───📁config.yaml                           # configuration for metaWEPP and WEPP
└───📁data                                       # [metaWEPP Generated/User Provided] Contains data to analyze 
     ├───📁pathogens_for_wepp                    # [metaWEPP Generated/User Provided] Pathogens for Variant Analysis with WEPP
          ├───📁RSV_A                   
               ├───RSV_A_ref_genome.fa     
               ├───RSV_A_mat.pb.gz
          ├───📁Rugose_Virus
     ...
     ├───📁simulated_metagenomic_sample          # [User Provided] Contains metagenomic sample for analysis
          ├───sample_R1.fastq.gz      
          ├───sample_R2.fastq.gz
     ├───📁real_metagenomic_sample               # [User Provided] Contains metagenomic sample for analysis
          ├───sample_R1.fastq.gz      
          ├───sample_R2.fastq.gz
...
└───📁results                                    # [metaWEPP Generated] 
      ├───📁real_metagenomic_sample
           ├───📁Rugose_Virus
                ├───Rugose_Virus_R1.fastq.gz         
                ├───Rugose_Virus_R2.fastq.gz             
           ├───📁SARS_COV_2
...
metaWEPP requires the following arguments, either through the 'config/config.yaml'.

1. `SEQUENCING_TYPE` - Sequencing read type (s:Illumina single-ended, d:Illumina double-ended, or n:ONT long reads)
2. `PRIMER_BED` - BED file for primers, which should be present in the `WEPP/primers` directory.
3. `MIN_AF` - Alleles with an allele frequency below this threshold in the reads will be masked.
4. `MIN_DEPTH` - Sites with read depth below this threshold will be masked.
5. `MIN_Q` - Alleles with a Phred score below this threshold in the reads will be masked.
6. `DASHBOARD_ENABLED` - Enable WEPP dashboard
7. `PATHOGENS_FOR_DASHBOARD` - Pathogens for WEPP dashboard(need to match to the pathogen directory name in pathogen_for_wepp)
8. `MIN_PROP` - Minimum proportion of haplotypes of haplotypes (Wastewater Samples: 0.5%, Clinical Samples: 5%).
9. `MIN_LEN` -  Minimum read length to be considered after ivar trim (Default: 80).
   
Arguments required to run WEPP. These may vary depending on the analysis settings.

11. `PATHOGENS` - List of pathogens have custom WEPP settings. Anything not listed here uses the default WEPP settings.
12. `CLADE_LIST` - Comma-separated list of clade annotation systems. Each element matches the pathogens in the order specified by PATHOGENS.
13. `CLADE_IDX` - Comma-separated list of clade index each pathogen should use. Each element matches the pathogens in the order specified by PATHOGENS.
14. `MIN_DEPTH_FOR_WEPP`  - Comma-separated list of minimum depth threshold for calling haplotypes in WEPP. Each element matches the pathogens in the order specified by PATHOGENS.
